...
export async function downloadDatabase(
  ...
  try {
    return await download(
      ...
      }/repos/${repoName}/code-scanning/codeql/databases/${language}`,
      `${repoId}.zip`,
      authHeader,
      "application/zip"
    );
...
export function getBqrsResultCount(bqrsInfo: BQRSInfo): number {
  ...
  throw new Error(
    `BQRS does not contain any result sets matching known names. Expected one of ${KNOWN_RESULT_SET_NAMES.join(
      " or "
    )} but found ${resultSetNames.join(", ")}`
  );
}
